...
import type { DBSensor, SensorData } from '@/types';
...
export async function addSensor(data: AddSensorFormData): Promise<{ success: boolean; message?: string; sensor?: DBSensor, errorField?: keyof AddSensorFormData }> {
  ...
  try {
    ...
    // Basic unique constraint check (though not explicitly defined in schema for sensors besides ID)
    // if (error.code === 'SQLITE_CONSTRAINT_UNIQUE') {
    //   return { success: false, message: 'A sensor with this configuration might already exist.', errorField: 'name' };
    // }
...
export async function updateSensor(id: string, data: UpdateSensorFormData): Promise<{ success: boolean; message?: string; sensor?: DBSensor, errorField?: keyof UpdateSensorFormData }> {
  ...
  try {
    ...
    // Note: currentValue and lastTimestamp are not updated by this action.
    // They are typically updated by incoming sensor readings.
    ...
    // Fetch the updated sensor to return its full state, excluding currentValue and lastTimestamp as they weren't changed here
